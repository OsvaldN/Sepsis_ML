...
pth = '/home/osvald/Projects/Diagnostics/CinC_data/MMtensors/'
...
def data_process(dataset, missing_markers=True, length_bins=True, class_count=True):
    '''
    ...
    if missing_markers:
        for i,pt in enumerate(dataset):
            markers = np.isnan(pt[:40,:]).astype(int)
            pt = np.concatenate((markers, np.nan_to_num(pt)), axis=0)
            dataset[i] = pt.T
    else:
...
#train_data = load_data(raw_dir, limit=5)
